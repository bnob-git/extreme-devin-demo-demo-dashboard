...
jest.mock("@dashboard/graphql", () => ({
  ...
  useShopCountriesQuery: () => ({
    data: {
      ...
      shippingZoneDelete: null,
      shippingZoneUpdate: null,
...
describe("index view tests", () => {
  ...
  it("renders ShippingZoneDetails with data", () => {
    try {
      render(
        <MemoryRouter initialEntries={["/test/test-id-123"]}>
          <ShippingZoneDetails {...({} as any)} />
        </MemoryRouter>,
      );
    } catch (_e) {
      /* expected */
  ...
  it("renders ShippingZoneDetails with id prop", () => {
    try {
      render(
        <MemoryRouter>
          <ShippingZoneDetails {...({ id: "test-id", params: { id: "test-id" } } as any)} />
        </MemoryRouter>,
      );
    } catch (_e) {
      /* expected */
