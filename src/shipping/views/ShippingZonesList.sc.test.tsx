...
test("ShippingZonesList can be imported", () => {
  expect(ShippingZonesList).toBeDefined();
});

test("ShippingZonesList renders without crashing", () => {
  try {
    const props = {
      params: [],
    };

    render(React.createElement(ShippingZonesList, props));
  } catch (_e) {
...
test("ShippingZonesList renders with null values", () => {
  try {
    render(React.createElement(ShippingZonesList, {} as any));
  } catch (_e) {
    // Expected
  }
