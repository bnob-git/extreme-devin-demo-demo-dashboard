...
jest.mock("use-react-router", () => ({
  ...
  default: () => ({
    ...
    location: { pathname: "/", search: "", hash: "" },
    match: { params: { id: "test-id" }, path: "/", url: "/", isExact: true },
  }),
}));
...
test("LoginView can be imported", () => {
  expect(LoginView).toBeDefined();
});

test("LoginView renders without crashing", () => {
  try {
    const props = {
      params: [],
    };

    render(React.createElement(LoginView, props));
  } catch (_e) {
...
test("LoginView renders with null values", () => {
  try {
    render(React.createElement(LoginView, {} as any));
  } catch (_e) {
    // Expected
  }
