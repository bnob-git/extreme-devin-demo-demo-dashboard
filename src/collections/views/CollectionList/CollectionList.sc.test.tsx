...
test("CollectionList can be imported", () => {
  expect(CollectionList).toBeDefined();
});

test("CollectionList renders without crashing", () => {
  try {
    const props = {
      params: [],
    };

    render(React.createElement(CollectionList, props));
  } catch (_e) {
...
test("CollectionList renders with null values", () => {
  try {
    render(React.createElement(CollectionList, {} as any));
  } catch (_e) {
    // Expected
  }
