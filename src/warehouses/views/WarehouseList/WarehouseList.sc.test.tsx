...
test("WarehouseList can be imported", () => {
  expect(WarehouseList).toBeDefined();
});

test("WarehouseList renders without crashing", () => {
  try {
    const props = {
      params: [],
    };

    render(React.createElement(WarehouseList, props));
  } catch (_e) {
...
test("WarehouseList renders with null values", () => {
  try {
    render(React.createElement(WarehouseList, {} as any));
  } catch (_e) {
    // Expected
  }
