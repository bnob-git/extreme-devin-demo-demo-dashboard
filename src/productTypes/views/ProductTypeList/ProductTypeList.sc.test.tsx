...
test("ProductTypeList can be imported", () => {
  expect(ProductTypeList).toBeDefined();
});

test("ProductTypeList renders without crashing", () => {
  try {
    const props = {
      params: [],
    };

    render(React.createElement(ProductTypeList, props));
  } catch (_e) {
...
test("ProductTypeList renders with null values", () => {
  try {
    render(React.createElement(ProductTypeList, {} as any));
  } catch (_e) {
    // Expected
  }
