...
jest.mock("@dashboard/attributes/utils/data", () => ({
  ...
  mergeAttributeValueDeleteErrors: jest.fn((..._a: any[]) => createDeepMock()),
  mergeFileUploadErrors: jest.fn((..._a: any[]) => createDeepMock()),
}));
jest.mock("@dashboard/attributes/utils/handlers", () => ({
  __esModule: true,
  default: jest.fn((..._a: any[]) => createDeepMock()),
  handleDeleteMultipleAttributeValues: jest.fn((..._a: any[]) => createDeepMock()),
...
describe("useProductUpdateHandler deep-proxy tests", () => {
  beforeEach(() => {
  ...
  it("calls useProductUpdateHandler with deep proxy args", () => {
    try {
      const result = (useProductUpdateHandler as any)(createDeepMock(), createDeepMock());
  ...
  it("calls useProductUpdateHandler with override args", () => {
    try {
  ...
  it("calls useProductUpdateHandler with null args for error paths", () => {
    try {
      const result = (useProductUpdateHandler as any)(null, null);
  ...
  it("calls useProductUpdateHandler with array data", () => {
    try {
      const result = (useProductUpdateHandler as any)(
