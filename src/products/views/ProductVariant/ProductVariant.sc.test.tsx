...
test("ProductVariant can be imported", () => {
  expect(ProductVariant).toBeDefined();
});

test("ProductVariant renders without crashing", () => {
  try {
    const props = {
      variantId: "test-id",
      params: [],
    ...
    render(React.createElement(ProductVariant, props));
  } catch (_e) {
...
test("ProductVariant renders with null values", () => {
  try {
    render(React.createElement(ProductVariant, {} as any));
  } catch (_e) {
    // Expected
  }
