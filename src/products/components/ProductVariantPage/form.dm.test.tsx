...
jest.mock("@dashboard/products/utils/data", () => ({
  ...
  getAttributeInputFromVariant: jest.fn((...args: any[]) => args[0] ?? {}),
  getStockInputFromVariant: jest.fn((...args: any[]) => args[0] ?? {}),
}));
jest.mock("@dashboard/products/utils/handlers", () => ({
  __esModule: true,
  default: jest.fn((...args: any[]) => args[0] ?? {}),
  createMediaChangeHandler: jest.fn((...args: any[]) => args[0] ?? {}),
...
describe("form deep-mock tests", () => {
  ...
  it("renders ProductVariantUpdateForm with deep-mocked deps", () => {
    try {
      render(
        <MemoryRouter>
          <ProductVariantUpdateForm {...({ submit: jest.fn(() => Promise.resolve({})) } as any)} />
        </MemoryRouter>,
      );
    } catch (_e) {
      /* expected */
