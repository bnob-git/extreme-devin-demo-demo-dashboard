...
test("ProductCreateForm can be imported", () => {
  expect(ProductCreateForm).toBeDefined();
});

test("ProductCreateForm renders without crashing", () => {
  try {
    const props = {} as any;

    render(React.createElement(ProductCreateForm, props));
  } catch (_e) {
...
test("ProductCreateForm renders with null values", () => {
  try {
    render(React.createElement(ProductCreateForm, {} as any));
  } catch (_e) {
    // Expected
  }
