...
jest.mock("@dashboard/hooks/useWizard", () => ({
  __esModule: true,
  default: (..._a: any[]) => createDeepMock(),
  useWizard: (..._a: any[]) => createDeepMock(),
}));
...
describe("ProductExportDialog hybrid render tests", () => {
  beforeEach(() => {
  ...
  it("renders ProductExportDialog with deepMock props", () => {
    try {
      render(
        <MemoryRouter>
          <ProductExportDialog {...(createDeepMock() as any)} />
        </MemoryRouter>,
      );
    } catch (_e) {
      /* expected */
  ...
  it("renders ProductExportDialog with override props", () => {
    ...
    try {
      render(
        <MemoryRouter>
          <ProductExportDialog {...(props as any)} />
        </MemoryRouter>,
      );
    } catch (_e) {
      /* expected */
