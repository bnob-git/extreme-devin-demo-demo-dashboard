...
jest.mock("@dashboard/components/UserAvatar", () => ({
  __esModule: true,
  default: jest.fn((..._a: any[]) => createDeepMock()),
  UserAvatar: () => null,
}));
...
jest.mock("@dashboard/misc", () => ({
  __esModule: true,
  default: jest.fn((..._a: any[]) => createDeepMock()),
  getUserInitials: jest.fn((..._a: any[]) => createDeepMock()),
...
describe("AssignMembersDialog deep-proxy tests", () => {
  beforeEach(() => {
  ...
  it("renders AssignMembersDialog with deep proxy props", () => {
    try {
      render(
        <MemoryRouter>
          <AssignMembersDialog {...(createDeepMock() as any)} />
        </MemoryRouter>,
      );
    } catch (_e) {
      /* expected */
  ...
  it("renders AssignMembersDialog with override props", () => {
    try {
      ...
      render(
        <MemoryRouter>
          <AssignMembersDialog {...(mockProps as any)} />
        </MemoryRouter>,
      );
    } catch (_e) {
      /* expected */
