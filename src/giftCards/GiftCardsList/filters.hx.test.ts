...
jest.mock("@dashboard/components/Filter/types", () => {
  try {
    return jest.requireActual("@dashboard/components/Filter/types");
  } catch {
    return new Proxy(
      {},
      {
...
jest.mock("@dashboard/utils/filters", () => {
  try {
    return jest.requireActual("@dashboard/utils/filters");
  } catch {
    return new Proxy(
      {},
      {
