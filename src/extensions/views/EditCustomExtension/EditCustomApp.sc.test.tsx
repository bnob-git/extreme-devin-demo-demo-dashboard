...
test("EditCustomExtension can be imported", () => {
  expect(EditCustomExtension).toBeDefined();
});

test("EditCustomExtension renders without crashing", () => {
  try {
    const props = {
      id: "test-id",
      params: [],
      token: undefined as any,
      onTokenClose: jest.fn(),
    };

    render(React.createElement(EditCustomExtension, props));
  } catch (_e) {
...
test("EditCustomExtension renders with null values", () => {
  try {
    render(React.createElement(EditCustomExtension, {} as any));
  } catch (_e) {
    // Expected
  }
