...
test("CategoryList can be imported", () => {
  expect(CategoryList).toBeDefined();
});

test("CategoryList renders without crashing", () => {
  try {
    const props = {
      params: [],
    };

    render(React.createElement(CategoryList, props));
  } catch (_e) {
...
test("CategoryList renders with null values", () => {
  try {
    render(React.createElement(CategoryList, {} as any));
  } catch (_e) {
    // Expected
  }
