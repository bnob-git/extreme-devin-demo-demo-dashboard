...
test("VoucherList can be imported", () => {
  expect(VoucherList).toBeDefined();
});

test("VoucherList renders without crashing", () => {
  try {
    const props = {
      params: [],
    };

    render(React.createElement(VoucherList, props));
  } catch (_e) {
...
test("VoucherList renders with null values", () => {
  try {
    render(React.createElement(VoucherList, {} as any));
  } catch (_e) {
    // Expected
  }
