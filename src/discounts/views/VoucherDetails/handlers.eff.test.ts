...
jest.mock("@dashboard/discounts/types", () => {
  const proxy: any = new Proxy(function () {}, {
    get: (_: any, p: string) => {
      if (p === "__esModule") return true;
...
describe("handlers", () => {
  it("can call createUpdateHandler", () => {
    expect(typeof createUpdateHandler).toBe("function");

    const result = createUpdateHandler({});

    expect(result).toBeDefined();
  });

  it("handles createUpdateHandler with no args", () => {
    expect(typeof createUpdateHandler).toBe("function");
  });
});
