...
test("AttributeList can be imported", () => {
  expect(AttributeList).toBeDefined();
});

test("AttributeList renders without crashing", () => {
  try {
    const props = {
      params: [],
    };

    render(React.createElement(AttributeList, props));
  } catch (_e) {
...
test("AttributeList renders with null values", () => {
  try {
    render(React.createElement(AttributeList, {} as any));
  } catch (_e) {
    // Expected
  }
