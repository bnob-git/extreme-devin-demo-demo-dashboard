...
jest.mock("@dashboard/auth/hooks/useUserPermissions", () => ({
  ...
  useUserPermissions: (..._a: any[]) => createDeepMock(),
}));
...
describe("CustomerListDatagrid deep-proxy tests", () => {
  beforeEach(() => {
  ...
  it("renders CustomerListDatagrid with deep proxy props", () => {
    try {
      render(
        <MemoryRouter>
          <CustomerListDatagrid {...(createDeepMock() as any)} />
        </MemoryRouter>,
      );
    } catch (_e) {
      /* expected */
  ...
  it("renders CustomerListDatagrid with override props", () => {
    try {
      ...
      render(
        <MemoryRouter>
          <CustomerListDatagrid {...(mockProps as any)} />
        </MemoryRouter>,
      );
    } catch (_e) {
      /* expected */
