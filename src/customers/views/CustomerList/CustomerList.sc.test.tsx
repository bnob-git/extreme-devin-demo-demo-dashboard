...
test("CustomerList can be imported", () => {
  expect(CustomerList).toBeDefined();
});

test("CustomerList renders without crashing", () => {
  try {
    const props = {
      params: [],
    };

    render(React.createElement(CustomerList, props));
  } catch (_e) {
...
test("CustomerList renders with null values", () => {
  try {
    render(React.createElement(CustomerList, {} as any));
  } catch (_e) {
    // Expected
  }
