...
test("OrderList can be imported", () => {
  expect(OrderList).toBeDefined();
});

test("OrderList renders without crashing", () => {
  try {
    const props = {
      params: [],
    };

    render(React.createElement(OrderList, props));
  } catch (_e) {
...
test("OrderList renders with null values", () => {
  try {
    render(React.createElement(OrderList, {} as any));
  } catch (_e) {
    // Expected
  }
