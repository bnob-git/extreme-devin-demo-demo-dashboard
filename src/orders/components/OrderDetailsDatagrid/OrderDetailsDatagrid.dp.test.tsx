...
jest.mock("@dashboard/products/urls", () => ({
  __esModule: true,
  default: jest.fn((..._a: any[]) => createDeepMock()),
  productPath: jest.fn((..._a: any[]) => createDeepMock()),
}));
...
describe("OrderDetailsDatagrid deep-proxy tests", () => {
  beforeEach(() => {
  ...
  it("calls OrderDetailsDatagrid with deep proxy args", () => {
    try {
      const result = (OrderDetailsDatagrid as any)(createDeepMock());
  ...
  it("calls OrderDetailsDatagrid with null args for error paths", () => {
    try {
      const result = (OrderDetailsDatagrid as any)(null);
  ...
  it("renders OrderDetailsDatagrid with deep proxy props", () => {
    try {
      render(
        <MemoryRouter>
          <OrderDetailsDatagrid {...(createDeepMock() as any)} />
        </MemoryRouter>,
      );
    } catch (_e) {
      /* expected */
  ...
  it("renders OrderDetailsDatagrid with override props", () => {
    try {
      ...
      render(
        <MemoryRouter>
          <OrderDetailsDatagrid {...(mockProps as any)} />
        </MemoryRouter>,
      );
    } catch (_e) {
      /* expected */
